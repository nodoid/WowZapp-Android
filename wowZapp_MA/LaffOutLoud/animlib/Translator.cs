		...
		public static string MakePlaybackCode (Animation Anim, int ScreenWidth, int ScreenHeight, RenderPlatformTypes RenderPlatformType, string JavaScript = "")
		{
			if (Anim.ScreenObjects.Count > 0) {
				...
				foreach (ScreenObject Item in Anim.ScreenObjects) {
					...
					float ItemSizeX = Item.Size.X * XPixelsOnePercent;
					float ItemSizeY = Item.Size.Y * YPixelsOnePercent;
                    float ItemInitialPositionX = (Item.InitialPosition.X * XPixelsOnePercent) + (ItemSizeX / 2);
                    float ItemInitialPositionY = (Item.InitialPosition.Y * YPixelsOnePercent) + (ItemSizeY / 2);
					...
					case ScreenObjectTypes.FreehandPath:
                        ...
                        RenderingCode.Append("obj_").Append(IdCounter).Append(".position = new Point(").AppendFormat(ci, "{0}", ItemInitialPositionX).Append(",").AppendFormat(ci, "{0}", ItemInitialPositionY).Append(");").AppendLine();
					...
					case ScreenObjectTypes.Image:
						...
						RenderingCode.Append ("var obj_").Append (IdCounter).Append ("_rotation = 0;").AppendLine ();
						RenderingCode.Append ("var obj_").Append (IdCounter).Append ("_scaling = 0;").AppendLine ();
						RenderingCode.Append ("obj_").Append (IdCounter).Append (".scale((").AppendFormat (ci, "{0}", ItemSizeX).Append (" / ").Append ("obj_").Append (IdCounter).Append (".bounds.width),")
							.Append ("(").AppendFormat (ci, "{0}", ItemSizeY).Append (" / ").Append ("obj_").Append (IdCounter).Append (".bounds.height));").AppendLine ();
						RenderingCode.Append ("obj_").Append (IdCounter).Append (".position = new Point(").AppendFormat (ci, "{0}", ItemInitialPositionX).Append (",").AppendFormat (ci, "{0}", ItemInitialPositionY).Append (");").AppendLine ();
					...
					if (Item.Transitions.Count > 0) {
						foreach (Transition step in Item.Transitions) {
							if (step.TransitionType == TransitionTypes.Move) {
								float stepStartPositionX = (step.StartPosition.X * XPixelsOnePercent) + (Item.Size.X / 2);
								float stepStartPositionY = (step.StartPosition.Y * YPixelsOnePercent) + (Item.Size.Y / 2);
								float stepEndPositionX = (step.EndPosition.X * XPixelsOnePercent) + (Item.Size.X / 2);
								float stepEndPositionY = (step.EndPosition.Y * YPixelsOnePercent) + (Item.Size.Y / 2);
								...
								AnimationCode.Append ("obj_").Append (IdCounter).Append (".position = new Point( ").AppendFormat (ci, "{0}", stepStartPositionX).Append (" + (time_passed * ").AppendFormat (ci, "{0}", XPercentile)
									.Append ("),").AppendFormat (ci, "{0}", stepStartPositionY).Append (" + (time_passed * ").AppendFormat (ci, "{0}", YPercentile).Append ("));").AppendLine ();
								AnimationCode.Append ("}").AppendLine ();
							}
							
							if (step.TransitionType == TransitionTypes.Scale) {
								float stepStartSizeX = step.StartSize.X * XPixelsOnePercent;
								float stepStartSizeY = step.StartSize.Y * YPixelsOnePercent;
								float stepEndSizeX = step.EndSize.X * XPixelsOnePercent;
								float stepEndSizeY = step.EndSize.Y * YPixelsOnePercent;
								...
								AnimationCode.Append ("if( event.time >= ").AppendFormat (ci, "{0}", step.StartsAt).Append (" && event.time <= ").AppendFormat (ci, "{0}", EndsAt).Append (") {").AppendLine ();
								AnimationCode.Append ("var time_passed = (event.time - ").AppendFormat (ci, "{0}", step.StartsAt).Append (") / ").AppendFormat (ci, "{0}", TimePercentile).Append (";").AppendLine ();
								AnimationCode.Append ("obj_").Append (IdCounter).Append (".scale(((").AppendFormat (ci, "{0}", stepStartSizeX).Append (" + (time_passed * ").AppendFormat (ci, "{0}", XPercentile).Append (")) / ").Append ("obj_").Append (IdCounter).Append (".bounds.width),")
									.Append ("((").AppendFormat (ci, "{0}", stepStartSizeY).Append (" + (time_passed * ").AppendFormat (ci, "{0}", YPercentile).Append (")) / ").Append ("obj_").Append (IdCounter).Append (".bounds.height));").AppendLine ();
								AnimationCode.Append ("}").AppendLine ();
							}
