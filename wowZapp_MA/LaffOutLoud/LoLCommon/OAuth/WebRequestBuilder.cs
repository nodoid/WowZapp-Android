namespace OAuthLib
{
    ...
    public sealed class WebRequestBuilder
    {
        ...
        public WebRequestBuilder(Uri requestUri, HTTPVerb verb, String UserAgent, NetworkCredential NetworkCredentials)
        {
			...
			object scopeParams = null;
			if (this.Parameters.TryGetValue("scope", out scopeParams))
			{

				string paramVal = scopeParams as String;
				if (!string.IsNullOrEmpty(paramVal))
				{
					paramVal = paramVal.Replace("+", " ");
					this.Parameters["scope"] = (object)paramVal;
				}//end if
...
//                }
        ...
        private void AddQueryStringParametersToUri()
        {
			...
			foreach (KeyValuePair<string, object> item in fieldsToInclude)
            {
				if( item.Value.GetType() == typeof(string) )
				{
					...
					Console.WriteLine("Request parameters: {0}", requestParametersBuilder);
        ...
        public void SetupOAuth()
        {
			...
			if (!string.IsNullOrEmpty(this.Tokens.ConsumerSecret))
			{
            ...
            {
                this.Parameters["oauth_token"] = this.Tokens.AccessToken;
            }
...
//                               select p;
			List<KeyValuePair<string, object>> paramsSorted = 
...
//                                   select p;
			Dictionary<string, object> sortedParameters = 
